...
if menu == "首頁":
    ...
    with t3:
        ...
        with exp2:
            st.caption("💡 系統支援多包裹合併計算。無單號訂單：匯入狀態、單號與運費。已存單號訂單：尋找對應的物流單號，【僅更新並重新計算運費】，不覆蓋狀態。")
            ...
            if uploaded_logi and st.button("🚀 執行【物流資訊】比對更新", type="primary", key="btn_update_logi"):
                try:
                    import re
                    # 🌟 1. 強制以字串讀取，避免 Excel 將物流單號轉為科學記號
                    ...
                    if not has_oid and not has_logi_num:
                        ...
                        # 🌟 2. 預先處理 Excel 檔案的所有列，轉換為乾淨的字典清單以供快速比對
                        excel_rows = []
                        for idx, row in df_logi.iterrows():
                            # 清洗訂單編號
                            oid = str(row['訂單編號']).strip() if has_oid else ""
                            if oid.lower() in ['nan', 'none', '']: oid = ""
                            if oid.endswith('.0'): oid = oid[:-2]
                            
                            # 清洗物流編號 (過濾中文字、結尾.0，並用斜線拆分)
                            logis_set = set()
                            raw_logi = str(row['物流編號']).strip() if has_logi_num else ""
                            if raw_logi.lower() not in ['nan', 'none', '']:
                                raw_logi = re.sub(r'物流編號|物流单号|承运单号|:|：|\s', '', raw_logi)
                                if raw_logi.endswith('.0'): raw_logi = raw_logi[:-2]
                                for x in re.split(r'[/,，、]+', raw_logi):
                                    if x.strip(): logis_set.add(x.strip())
                            ...
                            status_val = str(row['狀態']).strip() if has_status else ""
                            date_val = str(row['取貨日期']).strip() if has_date else ""
                        ...
                        with get_db() as conn:
                            cursor = conn.cursor()
                            cursor.execute("SELECT 訂單編號, 包裹應收, 商品成本, 物流運費, 物流運費_RMB, 物流編號, 取貨狀態 FROM customer_orders")
                            db_orders = cursor.fetchall()
                            
                            for db_row in db_orders:
                                db_oid, db_rev, db_cost, db_ship, db_ship_rmb, db_logis, db_status = db_row
                                db_oid_str = str(db_oid).strip()
                                db_logis_str = str(db_logis).strip() if db_logis else ""
                                
                                # 將資料庫裡的 A/B/C 單號拆開
                                db_logis_set = set([x.strip() for x in re.split(r'[/,，、\s]+', db_logis_str) if x.strip()])
                                
                                matched_excel_rows = []
                                # 尋找 Excel 中對應這筆訂單的所有行 (不論它是1行還是拆成3行)
                                for erow in excel_rows:
                                    match = False
                                    if db_oid_str and erow['oid'] == db_oid_str:
                                        match = True
                                    elif db_logis_set and erow['logis_set'] and db_logis_set.intersection(erow['logis_set']):
                                        # 只要 Excel 裡的單號有出現在資料庫裡，就抓出來算運費
                                        match = True
                                        
                                    if match:
                                        matched_excel_rows.append(erow)
                                        
                                if not matched_excel_rows:
                                    continue # 這張訂單不在這次的表格裡，跳過
                                    
                                # 🌟 4. 運費結算：加總 Excel 裡對應到的所有包裹運費
                                total_matched_fee_rmb = sum(r['fee'] for r in matched_excel_rows)
                                
                                needs_update = False
                                updates = ["物流運費=?", "物流運費_RMB=?", "出貨成本=?", "訂單損益=?"]
                                
                                new_fee_twd = total_matched_fee_rmb * exchange_rate
                                calc_ship = float(db_cost if db_cost else 0.0) + new_fee_twd
                                calc_profit = float(db_rev if db_rev else 0.0) - calc_ship
                                params = [new_fee_twd, total_matched_fee_rmb, calc_ship, calc_profit]
                                
                                if not db_logis_str:
                                    # 👉 情境 A：資料庫原本【沒有單號】 -> 導入單號、狀態、運費
                                    all_new_logis = []
                                    for r in matched_excel_rows:
                                        for l in r['logis_set']:
                                            if l not in all_new_logis: all_new_logis.append(l)
                                    new_logis_str = '/'.join(all_new_logis)
                                    
                                    last_status = matched_excel_rows[-1]['status']
                                    mapped_status = status_map.get(last_status, last_status) if last_status else None
                                    new_status = None
                                    if mapped_status in STATUS_LIST: new_status = mapped_status
                                    elif new_logis_str and not mapped_status: new_status = '配送中'
                                    
                                    new_date = matched_excel_rows[-1]['date'] if new_status == '簽收' else None
                                    
                                    updates.append("物流編號=?")
                                    params.append(new_logis_str)
                                    ...
                                    # 👉 情境 B：資料庫原本【已經有單號】 -> 只改運費！絕對不動狀態與編號
                                    # 只在算出來的新運費跟資料庫舊運費不同的時候，才執行覆蓋更新 (節省效能)
                                    db_old_fee_rmb = float(db_ship_rmb) if db_ship_rmb else 0.0
                                    if total_matched_fee_rmb != db_old_fee_rmb:
                                        needs_update = True
                                        
                                if needs_update:
                                    params.append(db_oid_str)
                                    cursor.execute(f"UPDATE customer_orders SET {', '.join(updates)} WHERE 訂單編號=?", tuple(params))
                                    update_count += 1
                                    
                            conn.commit()
                            
                        log_system_action("訂單明細", current_operator, "批量更新物流", f"比對更新了 {update_count} 筆訂單，匯率設定：{exchange_rate}")
                        st.success(f"✅ 成功為 {update_count} 筆訂單掛載了最新運費與物流資訊！(已防呆避免疊加，並精準核算)")
                        time.sleep(1.5); st.rerun()
                except Exception as e:
