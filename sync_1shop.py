...
def sync_1shop_orders():
    ...
    # ==========================================
    # 🟢 階段 1：[閃電連線] 只為了快閃去資料庫抓『匯率』與『成本』，抓完立刻關閉！
    # ==========================================
    ...
    try:
        ...
        print(f"💥 抓取基礎資料失敗: {str(e)}")
        if conn: conn.rollback()
        return
    finally:
        release_db_connection(conn) # 👈 🌟 抓完立刻還給連線池，不佔用資源！
    ...
    # ==========================================
    # 🟢 階段 2：[無連線狀態] 慢慢去 1shop 爬蟲，愛爬多久就爬多久！
    # ==========================================
    ...
    # 準備一個清單，把所有整理好的訂單資料先「暫存」在記憶體裡
    ready_to_insert_data = []
    ...
    for basic_order in orders_list:
        ...
        # 🌟 絕對關鍵：自動對抗 10秒10次限制 的減速防呆 (此時資料庫是關閉的，不用怕超時)
        time.sleep(1.2) 
    ...
    # ==========================================
    # 🟢 階段 3：[閃電連線] 全部抓完了！一次性把暫存資料寫入資料庫
    # ==========================================
    if not ready_to_insert_data:
        print("💡 沒有有效訂單需要寫入。")
        return

    print(f"📦 準備將 {len(ready_to_insert_data)} 筆訂單寫入資料庫...")
    
    conn2 = get_db_connection() # 👈 🌟 再借一次連線
    try:
        cursor2 = conn2.cursor()
        success_count = 0
        
        for data_tuple in ready_to_insert_data:
            cursor2.execute("""
            ...
            """, data_tuple)
            success_count += 1
            print(f"✅ 成功處理/更新訂單: {data_tuple[0]}")

        # 寫入系統日誌
        if success_count > 0:
            cursor2.execute("INSERT INTO system_logs (timestamp, module, operator, action_type, details) VALUES (%s, %s, %s, %s, %s)", 
        ...
        release_db_connection(conn2) # 👈 🌟 寫完立刻還回去！
